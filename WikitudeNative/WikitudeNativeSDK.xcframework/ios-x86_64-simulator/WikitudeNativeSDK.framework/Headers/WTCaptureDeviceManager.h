 ...
 */
@property (nonatomic, assign) GLuint                                    cameraRenderingTargetTexture WT_DEPRECATED_SINCE(8.0.0, "Implement `cameraFrameRenderingTargetTextureId` from the WTOpenGLESRenderingMode protocol instead.");

/**
 * @brief Use this method to define the point of interest which should be used to focus at.
 *
 * @param point A point in screen space such as returned from a UIGestureRecognizer. The unit is points not pixel.
 * @discussion This method handles device/interface orientation internally.
 */
- (void)focusAtPointOfInterest:(CGPoint)point;
 ...
 * @discussion This method handles device/interface orientation internally.
 */
- (void)exposeAtPointOfInterest:(CGPoint)point;

/**
 * @brief Use this method to define the point of interest which should be used to adopt the exposure
 *
 * @param point A point in screen space such as returned from a UIGestureRecognizer. The unit is points not pixel.
 * @param exposureMode The exposure to use for the device capturing the video.
 * @discussion This method handles device/interface orientation internally.
 */
- (void)exposeAtPointOfInterest:(CGPoint)point withMode:(AVCaptureExposureMode)exposureMode WT_DEPRECATED_SINCE(8.0.0, "Use exposeAtPointOfInterest: instead.");
