...
/**
 * @brief The target area at the camera's original frame.
 *
 * @return targetArea The area of the target in the camera's original frame
 */
 ...
 * This value only contains reliable values if the .wtc file or the cloud archive included physical image target heights.
 *
 * @param cameraFrameSize The resolution (in pixel) with which the camera is actually running (SD, HD, Full HD)
 *
 * @param surfaceSize The resolution (in pixel) of the current OpenGL or Metal render view
 *
 * @param angle The cameraToSurfaceAngle that describes the device rotation (landscape left, right, portrait, portrait upside down)
 *
 * @return targetArea The area of the target in the camera's original frame
 */
