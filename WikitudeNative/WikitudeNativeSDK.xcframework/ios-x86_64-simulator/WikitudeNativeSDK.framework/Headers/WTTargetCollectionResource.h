...
/**
 * @brief Use this method to start the actual loading process of the resource.
 *
 * @discussion This method either loads a local resource or starts a network connection to a server to download the resource, specified by URL.
 *
 * The loading process happens on a background thread
 *
 * @param completionHandler A completion handler that is called in case the resource is available or failed loading.
 */
- (void)load:(WTTargetCollectionResourceCompletionHandler)completionHandler;
